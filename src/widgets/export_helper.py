...
def export_data(parent, file_controller, source_filename, target_format):
    ...
    if not os.path.exists(source_file_path):
        QMessageBox.warning(parent, "Error", f"Could not find selected data file: {source_filename}")
        return
    ...
    if target_format == 'csv':
        ...
        QMessageBox.critical(parent, "Error", f"Unsupported export format '{target_format}'")
        return
    ...
    try:
        ...
        QMessageBox.information(parent, "Success", f"Data successfully exported to: {save_path}")

    except Exception as e:
        QMessageBox.critical(parent, "Export Error", f"An error occurred during export: {e}")
