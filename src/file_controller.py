...
class FileController:
    ...
    def __init__(self):
        ...
        self.particles_folder = self.config.get('particles_folder', 'particles/')
        self.original_frames_folder = self.config.get('original_frames_folder', 'original_frames/')
        self.annotated_frames_folder = self.config.get('annotated_frames_folder', 'annotated_frames/')
        self.rb_gallery_folder = self.config.get('rb_gallery_folder', 'rb_gallery')
        self.videos_folder = self.config.get('videos_folder', 'videos/')
    ...
    def save_particles_data(self, particles_df: pd.DataFrame, filename: str = "all_particles.csv") -> str:
    ...
    def save_trajectories_data(self, trajectories_df: pd.DataFrame, filename: str = "trajectories.csv") -> str:
    ...
    def get_total_frames_count(self) -> int:
        ...
        frame_files = [f for f in os.listdir(self.original_frames_folder) 
                      if f.startswith('frame_') and f.endswith('.jpg')]
        return len(frame_files)
    
    def export_data(self, source_filename: str, target_format: str, save_path: str) -> bool:
